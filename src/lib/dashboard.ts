...
export type Transaction = {
  amount: number;
  createdAt: string;
  object: { name: string; type: string } | null;
};
...
export type Learner = {
  ...
  totalUp: number;
  totalDown: number;
  validAudits: { nodes: AuditNode[] };
  failedAudits: { nodes: AuditNode[] };
};
export type DashboardData = {
  user: Learner[];
  moduleXP: Aggregate;
  projectXP: Aggregate;
  exerciseXP: Aggregate;
  piscineGoXP: Aggregate;
  piscineJsXP: Aggregate;
...
export type Project = {
  ...
  date: string | null;
  grade: number | null;
  status: "Passed" | "In progress" | "Retry";
};
...
export function getRank(level: number) {
  ...
  const percentage = next
    ? Math.min(100, ((safeLevel - current.level) / (next.level - current.level)) * 100)
    : 100;
...
export function getSkills(transactions: DashboardData["skillTransactions"]) {
  ...
  for (const transaction of transactions) {
    ...
    values.set(
      transaction.type,
      Math.max(values.get(transaction.type) || 0, Math.min(100, Math.max(0, transaction.amount))),
    );
  }
  return [...values]
    .map(([type, value]) => ({ name: readableName(type), value }))
...
export function getTimeline(transactions: Transaction[], period: Period, anchor: Date) {
  const valid = transactions
    .filter(
      (t) =>
        ...
        Date.parse(t.createdAt) <= anchor.getTime(),
    )
  ...
  const start =
    period === "all"
      ...
      : new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth() - (period === "6m" ? 5 : 2), 1));
  let cumulative = valid
    .filter((t) => Date.parse(t.createdAt) < start.getTime())
  ...
  for (const month = new Date(start); month <= end; month.setUTCMonth(month.getUTCMonth() + 1)) {
    ...
    cumulative += earned;
    points.push({
      label: month.toLocaleDateString("en", { month: "short", year: "2-digit", timeZone: "UTC" }),
