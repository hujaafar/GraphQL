...
export const AUTH_ENDPOINT = process.env.NEXT_PUBLIC_AUTH_URL || "https://learn.reboot01.com/api/auth/signin";
export const GRAPHQL_ENDPOINT = process.env.NEXT_PUBLIC_GRAPHQL_URL || "https://learn.reboot01.com/api/graphql-engine/v1/graphql";
...
export function getSession(): string | null {
  ...
  } catch { /* Unavailable storage is an unsigned-in session. */ }
  return null;
}

export function saveSession(token: unknown): void {
  if (!isValidSession(token)) throw new Error("The server returned an invalid session. Please try signing in again.");
...
export function clearSession(): void {
  try { sessionStorage.removeItem(TOKEN_KEY); localStorage.removeItem("authToken"); } catch { /* Storage may be disabled. */ }
}
...
export async function signIn(identifier: string, password: string): Promise<void> {
  const response = await fetch(AUTH_ENDPOINT, {
    method: "POST",
    headers: { Authorization: `Basic ${encodeCredentials(identifier.trim(), password)}`, "Content-Type": "application/json" },
    signal: AbortSignal.timeout(15000), cache: "no-store",
  });
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) throw new Error("That username or password isn’t correct. Please try again.");
