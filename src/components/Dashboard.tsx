            ...
            <Reveal id="audits" className="dashboard-section">
              ...
              <div className="audits-grid">
                ...
                <div className="panel audit-history">
                  <div className="panel-heading">
                    <h2>Audit history</h2>
                    <div className="segmented" aria-label="Audit result filter">
                  ...
                  <ul>
                    {filteredAudits
                      .slice(auditPageSafe * 5, auditPageSafe * 5 + 5)
                      .map((audit, index) => (
                        <li key={`${audit.group?.captainLogin}-${audit.group?.createdAt}-${index}`}>
                          <span
                            className={`audit-result ${audit.status === "Passed" ? "passed" : "failed"}`}
                          >
                            {audit.status === "Passed" ? <Check size={16} /> : <X size={16} />}
                          </span>
                          <div>
                            <strong>{audit.group?.captainLogin}</strong>
                            <span>Group created {dateLabel(audit.group!.createdAt)}</span>
                          </div>
                          <span
                            className={`status-badge ${audit.status === "Passed" ? "passed" : "retry"}`}
                          >
                            {audit.status}
                          </span>
                        </li>
